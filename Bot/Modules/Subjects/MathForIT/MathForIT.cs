...
{
    public class MathForIT : SP1XModuleBase
    {
        public MathForIT(IServiceProvider serviceProvider, IConfiguration configuration) : base(serviceProvider,
            configuration)
        {
        }
        ...
        public async Task MathForITCmd([Remainder] string argument)
        {
            ...
            {
                var mathAssignment = await _DataAccessLayer.GetAssignmentSubject(subject, arguments[0]);
                if (mathAssignment is null)
                {
                ...
                var mathEmbed = new SP1XEmbedBuilder()
                    .WithTitle(mathAssignment.Name)
                    .WithDescription(mathAssignment.Content)
                    .WithStyle(EmbedStyle.Information)
                    .Build();

                await Context.Channel.SendMessageAsync(embed: mathEmbed);
                return;
            }
